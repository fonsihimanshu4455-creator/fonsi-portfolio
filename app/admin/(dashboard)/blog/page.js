...
export default function BlogAdmin() {
  ...
  const togglePublish = async (p) => {
    ...
    await supabase.from("blog_posts").update(patch).eq("id", p.id);
    load();
  };
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        ...
        <Link href="/admin/blog/new" className="pill pill-red">
      ...
      {loading ? (
        ...
        <div className="card-elevated rounded-2xl p-10 text-center text-[color:var(--color-muted)] text-sm">
          No posts yet. Click <span className="text-[color:var(--color-red)]">New post</span> to draft one.
        </div>
      ) : (
                ...
                <tr key={r.id} className={clsx("border-b border-[color:var(--color-stroke)] last:border-0", !r.is_published && "opacity-60")}>
                  ...
                  <td className="px-4 py-3 text-right whitespace-nowrap">
                    <div className="inline-flex items-center gap-1">
                      ...
                      <Link href={`/admin/blog/${r.id}`} className="w-8 h-8 rounded-md hover:bg-white/5 inline-flex items-center justify-center" aria-label="Edit">
                        <Pencil size={14} />
                      </Link>
                      <button onClick={() => setConfirmDel(r)} className="w-8 h-8 rounded-md hover:bg-red-500/15 text-red-300 inline-flex items-center justify-center" aria-label="Delete">
                        <Trash2 size={14} />
                      </button>
                    </div>
                  </td>
      ...
      {confirmDel && (
        <div className="fixed inset-0 z-50 bg-black/60 flex items-center justify-center p-4">
          <div className="card-elevated rounded-2xl p-6 max-w-sm w-full">
            <div className="font-display font-bold text-lg mb-2">Delete this post?</div>
