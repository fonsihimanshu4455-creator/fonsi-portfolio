...
const BLANK = { icon_name: "zap", title: "", description: "", order: 0, is_visible: true };
const COLUMNS = [
  { key: "title", label: "Title" },
  { key: "icon_name", label: "Icon" },
];

export default function WebsiteFeaturesAdmin() {
  return (
    <CrudList
      ...
      renderForm={(d, set) => (
        <div className="grid sm:grid-cols-2 gap-4">
          ...
          <Field label="Icon">
            <Select value={d.icon_name || "zap"} onChange={(e) => set({ ...d, icon_name: e.target.value })}>
              {ICON_KEYS.map((k) => (
